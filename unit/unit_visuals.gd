...
func set_unit_logic(logic: UnitLogic) -> void:
	unit_logic = logic
	_connect_logic_signals()
	
# Protected initialization - can be overridden
func _initialize_visuals() -> void:
	base_y = sprite.position.y

# Protected signal connection - can be extended
func _connect_logic_signals() -> void:
	unit_logic.connect("spawned", Callable(self, "_on_spawned"))
	unit_logic.connect("took_damage", Callable(self, "_on_took_damage"))
	unit_logic.connect("died", Callable(self, "_on_died"))
	unit_logic.connect("status_applied", Callable(self, "_on_status_applied"))

# Virtual method for idle animation - can be overridden
func _process(delta: float) -> void:
	#print("super process")
	_update_idle_animation(delta)
	_update_status_visuals(delta)

# Protected method for idle animation - override this for different movement patterns
func _update_idle_animation(delta: float) -> void:
	time += delta
	# Default bobbing effect
	var base_offset = sin(time * 2.0) * 10.0
	sprite.position.y = base_y + base_offset

# ---------------------------
# Signal Handlers - Virtual methods
# ---------------------------
...
func _on_died(unit: UnitLogic) -> void:
	_play_death_effect()

func _on_status_applied(status_type) -> void:
	_apply_visual_status_effect(status_type)

# ---------------------------
# SPAWN EFFECT - Virtual method
# ---------------------------
...
# ---------------------------
# STATUS VISUAL EFFECTS - Virtual methods
# ---------------------------
func _apply_visual_status_effect(status_type) -> void:
	match status_type:
		UnitLogic.StatusType.SLOW:
			_apply_slow_visuals()
		UnitLogic.StatusType.POISON:
			_apply_poison_visuals()
		UnitLogic.StatusType.REVERSE:
			_apply_reverse_visuals()
		UnitLogic.StatusType.BURN:
			_apply_burn_visuals()
		_:
			print("Unknown status type: ", status_type)

# Protected status visual methods - can be overridden
func _apply_slow_visuals() -> void:
	active_visual_statuses[UnitLogic.StatusType.SLOW] = {
		"color": Color(0.0, 0.0, 1.0, 1.0) * 2, 
	}
	_update_status_color()

func _apply_poison_visuals() -> void:
	active_visual_statuses[UnitLogic.StatusType.POISON] = {
		"color": Color(0.3, 1.0, 0.3, 1.0), 
	}
	_update_status_color()

func _apply_reverse_visuals() -> void:
	active_visual_statuses[UnitLogic.StatusType.REVERSE] = {
		"color": Color.WHITE, 
	}
	_update_status_color()

func _apply_burn_visuals() -> void:
	active_visual_statuses[UnitLogic.StatusType.BURN] = {
		"color": Color(1.0, 0.4, 0.2,  1.0),  # Orange/red tint
	}
	_update_status_color()

# Virtual method for status color updates
func _update_status_color() -> void:
	...
	for status_type in active_visual_statuses:
		if unit_logic.active_statuses.has(status_type):  # Only if still active in logic
			var color = active_visual_statuses[status_type]["color"]
			final_color = final_color.lerp(color, 0.5)
			blend_factor += 0.3
	
	status_overlay_color = final_color
	_apply_color_to_sprites(final_color)

# Protected method for applying color - can be overridden for multiple sprites
...
func _calculate_status_movement_offset(delta: float) -> float:
	...
	if active_visual_statuses.has(UnitLogic.StatusType.SLOW) and unit_logic.active_statuses.has(UnitLogic.StatusType.SLOW):
		offset *= active_visual_statuses[UnitLogic.StatusType.SLOW].get("movement_modifier", 1.0)
...
func _update_status_visuals(delta: float) -> void:
	...
	var to_remove = []
	for status_type in active_visual_statuses:
		if not unit_logic.active_statuses.has(status_type):
